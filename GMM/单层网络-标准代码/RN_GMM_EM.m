function [XX,s]=RN_GMM_EM(M0,y0,m,T,t_max,t_max1,MU)
%GMM单层/符号网络重构
...
for tttt=1:10
    kkk=1;
    x_pre=repmat(XX0(:),[1,10]); %和前10步对比判断有无收敛，防止震荡
    for tt=1:t_max
        x_pre(:,1)=[];
        x_pre =[x_pre,XX(:)];%之前的解
        ...
        if  min(max(abs(x_pre-XX(:)),[],1))<MU && tt>=2
            break;
        end
